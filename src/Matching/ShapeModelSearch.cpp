    ...
    const SearchParams& params) const
{
    ...
    // COARSE angle step for LINEMOD: ~6 degrees
    // Since LINEMOD uses 8-bin (45 degree) quantization, coarser angle steps are acceptable
    constexpr double COARSE_ANGLE_STEP_DEG = 6.0;
    constexpr double COARSE_ANGLE_STEP = COARSE_ANGLE_STEP_DEG * PI / 180.0;
    ...
    // Search grid step - OR spreading with T=4 provides tolerance for ±2 pixel offset.
    // At level 2 (1/4 resolution), step=3 is acceptable for coarse search.
    // Refinement will find the exact position.
    constexpr int32_t stepSize = 3;

    // Get model features for top level
    const auto& modelFeatures = linemodFeatures_[startLevel];

    // Compute model bounds for search region validation
    int32_t modelMinX = 0, modelMaxX = 0, modelMinY = 0, modelMaxY = 0;
    for (const auto& f : modelFeatures) {
        modelMinX = std::min(modelMinX, static_cast<int32_t>(f.x));
        modelMaxX = std::max(modelMaxX, static_cast<int32_t>(f.x));
        modelMinY = std::min(modelMinY, static_cast<int32_t>(f.y));
        modelMaxY = std::max(modelMaxY, static_cast<int32_t>(f.y));
    }

    // LINEMOD coarse search threshold
    // With bit operation method (match/no-match), threshold should be lower
    // since we're doing exact orientation bit matching, not graded LUT scoring.
    // OR spreading provides spatial tolerance but not angular tolerance.
    // Use 0.7-0.8 for coarse search to find candidates, then refine.
    double coarseThreshold = std::max(0.75, params.minScore * 0.8);
    ...
    #pragma omp parallel
    {
        ...
        for (size_t ai = 0; ai < angles.size(); ++ai) {
            ...
            // OPTIMIZATION: Precompute rotated template for this angle (paper approach)
            // This moves cos/sin/round outside the position loop
            auto rotatedFeatures = LinemodPyramid::RotateFeatures(modelFeatures, angle);

            // Compute rotated bounds from precomputed features
            ...
            // Search all positions using SIMD batch scoring
            // Process 8 consecutive X positions at once (true SIMD)
            // Only keep positions aligned with stepSize to reduce candidates
            alignas(32) double scores8[8];

            for (int32_t y = searchYMin; y <= searchYMax; y += stepSize) {
                // 对齐到 stepSize 边界，确保 x % stepSize == searchXMin % stepSize
                int32_t xOffset = searchXMin % stepSize;
                int32_t x = searchXMin;

                // SIMD batch: 一次处理 8 个连续位置
    ...
    for (int32_t level = startLevel - 1; level >= 0; --level) {
        ...
        // Note: we'll combine with candidate's base angle
